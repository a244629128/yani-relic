...
export function etMonthStartUtc(year, month) {
  ...
  // Fallback if neither offset produced a clean midnight (extreme edge);
  // pick the EDT-ish guess. The query bounds will be slightly off but
  // events won't be dropped entirely.
  return new Date(Date.UTC(year, month - 1, 1, 4, 0, 0, 0));
}
