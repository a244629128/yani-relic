...
export async function getDailyStatsForMonth({ year, month }) {
  ...
  // Paginate (Codex HIGH #3) — Supabase JS client caps responses at ~1000
  // rows by default, so a busy month would silently undercount. Loop until
  // we get a short page.
  const PAGE = 1000;
  const events = [];
  let from = 0;
  while (true) {
    ...
    from += PAGE;
    // Safety stop — at ~50K events/month this loop has already covered
    // way more than a hobby shop will ever generate.
    if (from > 50000) {
      console.warn("[getDailyStatsForMonth] page cap reached at 50000");
      break;
    }
  }
