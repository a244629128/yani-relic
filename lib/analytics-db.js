...
// Shared paginated fetch — Supabase JS caps single .select() responses at
// ~1000 rows by default. Without paging, busy windows undercount silently.
// Caller passes a builder that returns the .from().select().filter() chain;
// we apply .range() per page and stop when a short page comes back.
async function fetchAllPaged(buildQuery, { pageSize = 1000, maxRows = 50000 } = {}) {
  const out = [];
  let from = 0;
  while (true) {
    ...
    from += pageSize;
    if (from > maxRows) {
      console.warn(`[analytics-db] page cap reached at ${maxRows}`);
      break;
    }
  }
  return out;
