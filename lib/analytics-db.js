...
export async function getProductStats({ days = 30 } = {}) {
  ...
  if (error) {
    ...
    return { stats: [], totals: { views: 0, depopClicks: 0, imageZooms: 0, uniqueVisitors: 0 } };
  }
  ...
  const stats = Array.from(byId.values()).map((r) => {
    const avgMs =
      r.durationsMs.length > 0
        ...
        : 0;
    const ctr = r.views > 0 ? r.depopClicks / r.views : 0;
    return {
      id: r.id,
      name: r.name,
      price: r.price,
