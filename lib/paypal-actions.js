...
import { getProduct, markProductSold, markProductAvailable } from "@/lib/products-db";
...
export async function createPayPalOrder(productId, buyerSessionId = null) {
  ...
  const chargeAmount = Number(product.effectivePrice || product.price);
    ...
    paypalOrder = await paypalRequest("/v2/checkout/orders", {
      ...
      body: {
        ...
        application_context: {
          ...
          user_action: "PAY_NOW",
          // Used only when PayPal falls back to same-window flow (e.g.,
          // some mobile browsers blocking popups). The Smart Button
          // popup ignores these. The buyer can navigate from /shop back
          // to the relic via the PayPal receipt link; this just gives
          // PayPal a safe destination so the flow can resolve.
          return_url: process.env.NEXT_PUBLIC_SITE_URL
            ? `${process.env.NEXT_PUBLIC_SITE_URL}/shop/${productId}`
            : `https://yani-relic.vercel.app/shop/${productId}`,
...
export async function getOrderForBuyer(orderId, sessionId) {
  ...
  return { ok: true, order: orderOut, product };
}
