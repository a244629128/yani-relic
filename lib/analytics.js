...
/**
 * Fired when a visitor clicks "Claim →" on a face-up flip-deck card on
 * the homepage. Top-of-funnel signal — how often does the deck drive
 * traffic to a relic's product page?
 */
export function trackFlipDeckClaim(productId) {
  if (!productId) return;
  sendEvent({ productId, eventType: "flip_deck_claim" });
}
