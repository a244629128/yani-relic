  ...
  try {
    ...
    if (isImage) {
      ...
      try {
        ...
        console.error("[uploadMediaFile] sharp failed:", err);
        return { ok: false, error: `Image processing failed: ${err.message || "unknown"}` };
      }
    ...
    if (upErr) {
      console.error("[uploadMediaFile] supabase upload error:", upErr);
    ...
    return { ok: false, error: err?.message || "Upload failed (server error)" };
  }
}
