...
export async function saveProduct(product) {
  ...
  if (row.depop_url) {
    if (row.depop_url.length > 512) {
      return { ok: false, error: "Depop URL is too long (max 512 chars)" };
    }
    if (!/^https:\/\/(www\.)?depop\.com\//i.test(row.depop_url)) {
      return {
        ok: false,
        error: "Depop URL must start with https://www.depop.com/ or https://depop.com/",
      };
    }
  }
...
export async function bulkApplyDiscount(percent) {
  ...
  // Per-row UPDATE instead of upsert. Postgres validates the INSERT row
  // (NOT NULL on name/description/etc.) before evaluating ON CONFLICT, so
  // a sparse upsert payload like {id, sale_price} fails with
  //   "null value in column 'name' violates not null constraint"
  // For 8 products this is ~8 queries totalling <100ms — fine. Done in
  // parallel via Promise.all.
  const results = await Promise.all(
    updates.map(async (u) => {
      const { error } = await sb
        .from("products")
        .update({ sale_price: u.sale_price })
        .eq("id", u.id)
        .eq("sold", false);
  ...
  if (failures.length === results.length) {
    ...
    console.error("[bulkApplyDiscount] all updates failed:", failures);
    return { ok: false, error: failures[0].error.message };
  }
  if (failures.length > 0) {
    console.warn(`[bulkApplyDiscount] ${failures.length}/${results.length} updates failed:`, failures);
  }
  ...
  return { ok: true, updated: results.length - failures.length };
}
